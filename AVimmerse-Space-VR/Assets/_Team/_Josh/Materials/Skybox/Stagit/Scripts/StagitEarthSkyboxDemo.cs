...
public class StagitEarthSkyboxDemo : MonoBehaviour
{
	...
	void Start ()
	{
		...
		sun.transform.rotation = Quaternion.Euler (sunPosition [0]);
	...
	void Update ()
	{
		...
		if (Input.GetKeyDown (KeyCode.N)) {
			...
			RenderSettings.skybox = skyBoxMaterial [currentSkyBoxIndex]; 
			topText = skyBoxMaterial [currentSkyBoxIndex].name;
            if (currentSkyBoxIndex <= sunPosition.Length) { 
			    sun.transform.eulerAngles = sunPosition[currentSkyBoxIndex];
            }
	...
	protected virtual void OnGUI ()
	{
		...
		if (string.IsNullOrEmpty (topText) == false) {
			DrawText ("Skybox[" + currentSkyBoxIndex + "] Name:" + topText + " (Press the [N] key for the next skybox) Use mouse scroll for zoom", TextAnchor.UpperCenter, 150);
		}
